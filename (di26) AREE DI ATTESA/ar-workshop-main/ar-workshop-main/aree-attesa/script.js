...
window.onload = () => {
    window.panel = $('.detail-panel', parent.document);
    window.img = $('.footer-button img', parent.document);

    window.titleElem = $('.footer-place-title', parent.document);
    window.distanceElem = $('.footer-place-distance', parent.document);
    window.descriptionElem = $('.detail-description', parent.document);
    window.detailTitle = $('.detail-title', parent.document);
    window.detailDistanceElem = $('.detail-distance', parent.document);
    window.detailAddressElem = $('.detail-address', parent.document);
    window.moreDetailsLink = $('.more-details', parent.document);
...
function closePanel() {
    ...
    window.panel.addClass('translate-y-full');
    window.moreDetailsLink.addClass('hidden');

    // update footer data
    setTimeout(() => {
        window.titleElem.text(window.closestPlace && (window.closestPlace.denom || ''));
        window.distanceElem.text(window.closestPlace && (window.closestPlace.distanceMsg || ''));
    }, 100);
}

function updatePanelData() {
    window.descriptionElem.text(window.closestPlace && (window.closestPlace.note || ''));
    window.detailTitle.text(window.closestPlace && (window.closestPlace.denom || ''));
    window.detailDistanceElem.text(window.closestPlace && (window.closestPlace.distanceMsg || ''));
...
/*
function confetti() {
  let confetti = $('#show-confetti');
  confetti.show();
  confetti.html('<iframe src="https://i.giphy.com/media/gKrbnqo25MlI2TUC78/giphy.webp" style="position:fixed; left:0; bottom:0; width:100%; height:100%; border:none; margin:0; padding:0; overflow:hidden; z-index:999999;">Your browser doesn\'t support iframes</iframe>');
